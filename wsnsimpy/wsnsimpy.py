...
class Node:
    ...
    ############################
    def __init__(self,sim,id,pos,scale):
        self.pos = pos
        self.vispos=list(map(lambda x:(x+100)*scale,self.pos))
        self.sim = sim
        self.id  = id
        self.logging = True
...
class Simulator:
    ...
    ############################
    def add_node(self,nodeclass,pos,scale):
        id = len(self.nodes)
        node = nodeclass(self,id,pos,scale)
