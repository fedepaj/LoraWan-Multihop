...
class Packet():
    @classmethod
    def frompacket(cls,p):
        return cls(p.sender,p.dest,p.freq,p.sf,p.bw,p.cr,p.txpow,p.distance,p.msg,p.ext,p.payload)
                    ...
                    if (per(packet.sf,packet.bw,packet.cr,packet.rssi,packet.pl) < random.uniform(0,1)):
                        ...
                        if packet.msg=='ack' or packet.msg=='jacc':
                            ...
                            if (self.now >= self.dutyCycle1p[chanlindex]):
                                ...
                                self.delayed_exec(2,self.scene.delshape,obj_id)
...
class RelayNode(LoraWANNode):
    ...
    def on_receive(self, packet, **kwargs):
        ...
        self.collisionWLine.append(Packet.frompacket(packet))
        if self.recId==1:
    ...
    def rx_end(self):
        for p in self.collisionWLine:
            if p.collided==1:
                ...
                line2 = self.scene.line(x+5,y-5,x-5,y+5,line="wsnsimpy:collision")
                self.delayed_exec(1,self.scene.delshape,line1)
                self.delayed_exec(1.5,self.scene.delshape,line2)
            if p.collided==0:
                if p.msg == 'jreq':
                    if self.EXT is not None:
                        self.send_jacc(p)
...
collMean=sum([rel.collNo for rel in rels])/len(rels)+sum([gw.collNo for gw in gws])/len(gws)
notConnected=0
for rel in rels:
...
for ed in eds:
    if ed.parent is None:
        notConnected+=1
print(tsx*tsy,":",xGrid*yGrid,":",args.gw,":",relNo,":",sended,":",received,":",collMean,":",sonsMean,":",notConnected,end="")
"""df=pd.DataFrame({
    'received':sum([len(gw.dataQ) for gw in gws]),
    'sended':sum([ed.sendedNo for ed in eds]),
    'sons':np.mean([])
})
df.plot(kind='scatter',x='ids',y='received',color='red')
plt.show()"""
#print(list(datas))